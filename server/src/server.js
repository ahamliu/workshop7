...
app.put('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
  ...
  if (fromUser === userId) {
    ...
    push["comments." + commentIdx + ".likeCounter"] = new ObjectID(userId)
      db.collection('feedItems').updateOne({ _id: new ObjectID(feedItemId) }, {$push: push}, function(err) {
        if (err) {
          res.status(500).send("Database error: " + err);
        } else {
          getFeedItem(new ObjectID(feedItemId), function(err, feedItem) {
...
app.delete('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
  ...
  if (fromUser === userId) {
    ...
    pull["comments." + commentIdx + ".likeCounter"] = new ObjectID(userId)
      db.collection('feedItems').updateOne({ _id: new ObjectID(feedItemId) }, {$pull: pull}, function(err) {
        if (err) {
          res.status(500).send("Database error: " + err);
        } else {
          getFeedItem(new ObjectID(feedItemId), function(err, feedItem) {
